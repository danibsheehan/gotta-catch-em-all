...
describe('PokeApiClient', () => {
  ...
  it('should GET type index', () => {
    let body: unknown;
    client.getTypeIndex().subscribe((b) => {
      body = b;
    });
  ...
  it('should encode path segments for type detail and pokemon', () => {
    let typeOk = false;
    client.getTypeDetail('a/b').subscribe(() => {
      typeOk = true;
    });
    ...
    let pokemonOk = false;
    client.getPokemon('x y').subscribe(() => {
      pokemonOk = true;
    });
    httpMock
      .expectOne('https://pokeapi.co/api/v2/pokemon/x%20y')
      .flush({ name: 'x y', sprites: {} as any, stats: [] });
