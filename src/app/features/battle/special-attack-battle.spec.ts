...
import { SPECIAL_ATTACK_LOSE_MESSAGE, SPECIAL_ATTACK_WIN_MESSAGE, resolveSpecialAttackBattle, } from './special-attack-battle';
...
describe('resolveSpecialAttackBattle', () => {
    ...
    it('returns null when choice has no special-attack stat', () => {
        expect(resolveSpecialAttackBattle({ ...choice, stats: [] }, opponent)).toBeNull();
    });

    it('returns null when opponent has no special-attack stat', () => {
        expect(resolveSpecialAttackBattle(choice, { ...opponent, stats: [] })).toBeNull();
    });
    ...
    it('declares the opponent the victor when special attack is lower', () => {
        const out = resolveSpecialAttackBattle({ ...choice, stats: [{ base_stat: 50, stat: { name: 'special-attack' } }] }, opponent);
    ...
    it('returns null when choice stats array is undefined', () => {
        expect(resolveSpecialAttackBattle({ ...choice, stats: undefined }, opponent)).toBeNull();
    });
});
