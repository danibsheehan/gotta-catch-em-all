...
def main() -> int:
    ...
    lines.extend(
        [
            ...
            "### CI",
            "",
            f"Required checks: [verify]({ci_href}) runs format/lint/audit/test/build "
            "as separate per-concern checks for every PR (Angular's AOT build already "
            "type-checks, so there's no separate typecheck check). Coverage tables are "
            "posted separately.",
            "",
            "---",
            "",
